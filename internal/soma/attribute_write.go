...
// length
func newAttributeWrite(length int) (string, *AttributeWrite) {
	w := &AttributeWrite{}
...
// it processes
func (w *AttributeWrite) RegisterRequests(hmap *handler.Map) {
	for _, action := range []string{
		msg.ActionAdd,
		msg.ActionRemove,
	} {
		hmap.Request(msg.SectionAttribute, action, w.handlerName)
	}
}
